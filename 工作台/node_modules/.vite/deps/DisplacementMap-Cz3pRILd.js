...
import { C as ZERO, Cn as std_exports, P as call, Sn as src_default, Ut as resolveBoundingBox, Vn as dot, Yt as sampleRemappedExpr, _r as vec3f, cr as data_exports, hn as unpremultiplyAlpha, hr as vec2f, l as setColorSpaceMode, m as transformEdges, n as ShaderContext, qn as max, rr as sin, s as createGpuUniformsMap, vr as vec4f, yr as f32, zn as cos } from "./Shader-BPX4e-H3.js";
import { t as require_jsx_runtime } from "./react_jsx-runtime.js";
import { t as require_react_dom } from "./react-dom.js";
//#region node_modules/shaders/dist/core/DisplacementMap-lv-qoLem.js
var DEG_TO_RAD = Math.PI / 180;
var dmDisplaceRG = src_default.fn([
	vec2f,
	vec4f,
	f32,
	f32
], vec2f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((uv, src, aspect, amount) => {
	"use gpu";
	const a = max(src.w, 1e-4);
	const dx = __tsover_div(__tsover_mul(__tsover_mul(__tsover_mul(__tsover_sub(__tsover_div(src.x, a), .5), 2), src.w), amount), aspect);
	const dy = __tsover_mul(__tsover_mul(__tsover_mul(__tsover_sub(__tsover_div(src.y, a), .5), 2), src.w), amount);
...
var dmDisplaceLuminance = src_default.fn([
	vec2f,
	vec4f,
	f32,
	f32,
	f32
], vec2f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((uv, src, aspect, amount, angleDeg) => {
	"use gpu";
	const a = max(src.w, 1e-4);
	const lum = dot(vec3f(__tsover_div(src.x, a), __tsover_div(src.y, a), __tsover_div(src.z, a)), vec3f(.2126, .7152, .0722));
	const scalar = __tsover_mul(__tsover_mul(__tsover_mul(__tsover_sub(lum, .5), 2), src.w), amount);
	const angleRad = __tsover_mul(angleDeg, DEG_TO_RAD);
	const dx = __tsover_div(__tsover_mul(scalar, cos(angleRad)), aspect);
	const dy = __tsover_mul(scalar, sin(angleRad));
	return vec2f(__tsover_add(uv.x, dx), __tsover_add(uv.y, dy));
}), {
	v: 1,
	name: void 0,
...
function channelModeOf(raw) {
	if (typeof raw === "number") return raw;
	return CHANNEL_MODES[raw] ?? 0;
}
var componentDefinition = {
	...
	props: {
		...
		channelMode: {
			...
			compileTime: true,
			transform: (value) => CHANNEL_MODES[value] ?? 0,
			ui: {
				type: "select",
				options: [{
					label: "Red / Green",
		...
		angle: {
			...
			ui: {
				...
				label: "Angle",
				group: "Effect",
				condition: { channelMode: "directional" }
			}
		},
		edges: {
			default: "mirror",
	...
	fragment: (params) => {
		const { uniforms, childNode, ctx, propValues, convertToTexture, getLayerTexture } = params;
		if (!childNode) return ZERO;
		const texture = convertToTexture(childNode);
		const source = getLayerTexture("source");
		if (!source) return call(unpremultiplyAlpha, "unpremultiplyAlpha", [texture.sample(ctx.uv)]);
		const edgeMode = propValues.edges ?? 0;
		const src = source.sample(ctx.uv);
		return call(unpremultiplyAlpha, "unpremultiplyAlpha", [sampleRemappedExpr(texture, channelModeOf(propValues.channelMode) === 1 ? call(dmDisplaceLuminance, "dmDisplaceLuminance", [
			ctx.uv,
			src,
			ctx.aspect,
			uniforms.amount,
...
//#endregion
//#region node_modules/shaders/dist/react/DisplacementMap.js
var DisplacementMap_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => DisplacementMap_default
});
...
//#endregion
export { DisplacementMap_exports as n, DisplacementMap_default as t };
