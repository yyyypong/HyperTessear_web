...
import { C as ZERO, Cn as std_exports, Jn as min, Ln as atan2, P as call, Pn as select, Sn as src_default, Ut as resolveBoundingBox, Wn as floor, cr as data_exports, d as transformBoolean, h as transformPosition, hr as vec2f, ir as smoothstep, it as floatE, l as setColorSpaceMode, n as ShaderContext, qn as max, s as createGpuUniformsMap, vr as vec4f, yr as f32 } from "./Shader-BPX4e-H3.js";
import { t as require_jsx_runtime } from "./react_jsx-runtime.js";
import { t as require_react_dom } from "./react-dom.js";
//#region node_modules/shaders/dist/core/RadialWipe-Bz19qvls.js
var DEG_TO_RAD = Math.PI / 180;
var TWO_PI = Math.PI * 2;
var transformDirection = (value) => {
	switch (value) {
		case "cw": return 0;
		case "ccw": return 1;
		case "both": return 2;
...
var radialWipeMask = src_default.fn([
	vec4f,
	vec2f,
	f32,
	vec2f,
	...
	f32,
	f32
], vec4f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((color, uv, aspect, center, startDeg, progress, feather, mode, invert) => {
	"use gpu";
	const rel = vec2f(__tsover_mul(__tsover_sub(uv.x, center.x), aspect), __tsover_sub(uv.y, __tsover_sub(1, center.y)));
	const ang = atan2(rel.y, rel.x);
	const start = __tsover_mul(startDeg, DEG_TO_RAD);
	let frac = __tsover_div(__tsover_sub(ang, start), TWO_PI);
	frac = __tsover_sub(frac, floor(frac));
	const ccwCoord = __tsover_sub(1, frac);
	const bothCoord = __tsover_mul(min(frac, __tsover_sub(1, frac)), 2);
	let coord = select(frac, ccwCoord, mode > .5);
	coord = select(coord, bothCoord, mode > 1.5);
	coord = select(coord, __tsover_sub(1, coord), invert > 0);
	const f = max(feather, 1e-4);
	const front = __tsover_sub(__tsover_mul(progress, __tsover_add(1, __tsover_mul(2, f))), f);
	const reveal = smoothstep(__tsover_sub(front, f), __tsover_add(front, f), coord);
...
var componentDefinition = {
	...
	fragment: ({ uniforms, childNode, ctx, propValues }) => {
		if (!childNode) {
			console.error("You must pass a child component into the Radial Wipe shader.");
			return ZERO;
		}
		const mode = transformDirection(propValues.direction ?? "cw");
		return call(radialWipeMask, "radialWipeMask", [
			childNode,
			ctx.uv,
			ctx.aspect,
			uniforms.center,
...
//#endregion
//#region node_modules/shaders/dist/react/RadialWipe.js
var RadialWipe_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => RadialWipe_default
});
...
//#endregion
export { RadialWipe_exports as n, RadialWipe_default as t };
