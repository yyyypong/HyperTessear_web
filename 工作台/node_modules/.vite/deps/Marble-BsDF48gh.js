...
import { Cn as std_exports, Ct as mixColorsLinear, Mt as noise_exports, P as call, Sn as src_default, Ut as resolveBoundingBox, a as animatedTime, c as colorSpaceOptions, cr as data_exports, f as transformColor, hr as vec2f, ir as smoothstep, kt as mxNoiseFloat2, l as setColorSpaceMode, n as ShaderContext, p as transformColorSpace, rr as sin, s as createGpuUniformsMap, wt as mixColorsVariants, yr as f32 } from "./Shader-BPX4e-H3.js";
//#region node_modules/shaders/dist/core/Marble-BSSMSrv8.js
var PI = Math.PI;
var marbleFactors = src_default.fn([
	vec2f,
	vec2f,
	f32,
	f32,
	f32,
	f32
], vec2f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((uv, viewport, scale, turbulence, seed, animTime) => {
	"use gpu";
	const aspect = __tsover_div(viewport.x, viewport.y);
	const au = __tsover_mul(uv.x, aspect);
	const av = uv.y;
	const n1 = mxNoiseFloat2(vec2f(__tsover_add(__tsover_mul(au, scale), seed), __tsover_add(__tsover_mul(av, scale), animTime)));
	const n2 = __tsover_mul(mxNoiseFloat2(vec2f(__tsover_add(__tsover_mul(__tsover_mul(au, scale), 2), __tsover_mul(seed, 1.7)), __tsover_add(__tsover_mul(__tsover_mul(av, scale), 2), __tsover_mul(animTime, .7)))), .5);
	const n3 = __tsover_mul(mxNoiseFloat2(vec2f(__tsover_add(__tsover_mul(__tsover_mul(au, scale), 4), __tsover_mul(seed, 3.1)), __tsover_add(__tsover_mul(__tsover_mul(av, scale), 4), __tsover_mul(animTime, 1.3)))), .25);
	...
	const marbleT = __tsover_add(__tsover_mul(sin(marbleCoord), .5), .5);
	const n4 = __tsover_add(__tsover_mul(mxNoiseFloat2(vec2f(__tsover_add(__tsover_mul(__tsover_mul(av, scale), 1.5), __tsover_mul(seed, 2)), __tsover_mul(__tsover_mul(au, scale), 1.2))), .3), .5);
	const t1 = smoothstep(.3, .6, marbleT);
	const t2 = smoothstep(.6, .9, marbleT);
	return vec2f(t1, __tsover_mul(t2, n4));
}), {
	v: 1,
	name: void 0,
	ast: {
...
//#endregion
//#region node_modules/shaders/dist/react/Marble.js
var Marble_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => Marble_default
});
...
var Marble_default = ShaderComponent;
//#endregion
export { Marble_exports as n, Marble_default as t };
