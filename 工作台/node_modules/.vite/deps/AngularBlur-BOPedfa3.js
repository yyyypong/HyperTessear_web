...
import { C as ZERO, Cn as std_exports, P as call, Sn as src_default, Ut as resolveBoundingBox, cr as data_exports, h as transformPosition, hn as unpremultiplyAlpha, hr as vec2f, it as floatE, l as setColorSpaceMode, n as ShaderContext, rr as sin, s as createGpuUniformsMap, yr as f32, zn as cos } from "./Shader-BPX4e-H3.js";
//#region node_modules/shaders/dist/core/AngularBlur-xU811GPM.js
var rawWeights = Array.from({ length: 32 }, (_, i) => {
	const t = (i / 31 - .5) * 2;
	return Math.exp(-.5 * t * t / .64);
});
var wSum = rawWeights.reduce((a, b) => a + b);
var BLUR_WEIGHTS = rawWeights.map((w) => w / wSum);
var angularBlurTapCoord = src_default.fn([
	vec2f,
	f32,
	vec2f,
	f32,
	f32
], vec2f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((center, intensity, uv, aspect, tapIndex) => {
	"use gpu";
	const centerPos = vec2f(center.x, __tsover_sub(1, center.y));
	const angleStep = __tsover_div(__tsover_mul(intensity, .005), 31);
	const angle = __tsover_mul(tapIndex, angleStep);
	const cosA = cos(angle);
	const sinA = sin(angle);
	const initX = __tsover_sub(uv.x, centerPos.x);
	const initY = __tsover_sub(uv.y, centerPos.y);
	const acX = __tsover_mul(initX, aspect);
	const acY = initY;
	const rotX = __tsover_sub(__tsover_mul(acX, cosA), __tsover_mul(acY, sinA));
	const rotY = __tsover_add(__tsover_mul(acX, sinA), __tsover_mul(acY, cosA));
...
var componentDefinition = {
	...
	fragment: ({ uniforms, childNode, ctx, convertToTexture }) => {
		...
		let total;
		for (let i = 0; i < 32; i++) {
			const coord = call(angularBlurTapCoord, "angularBlurTapCoord", [
				uniforms.center,
				uniforms.intensity,
				ctx.uv,
				ctx.aspect,
...
var AngularBlur_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => AngularBlur_default
});
...
//#endregion
export { AngularBlur_exports as n, AngularBlur_default as t };
