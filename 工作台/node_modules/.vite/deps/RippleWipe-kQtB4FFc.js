...
var rippleWipeMask = src_default.fn([
	...
	f32,
	f32
], vec4f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((color, uv, aspect, center, rings, progress, feather, invert) => {
	"use gpu";
	const cx = __tsover_mul(center.x, aspect);
	const cy = __tsover_sub(1, center.y);
	const dist = length(vec2f(__tsover_sub(__tsover_mul(uv.x, aspect), cx), __tsover_sub(uv.y, cy)));
	const maxDist = length(vec2f(max(cx, __tsover_sub(aspect, cx)), max(cy, __tsover_sub(1, cy))));
	const rNorm = __tsover_div(dist, max(maxDist, 1e-4));
	const n = max(rings, 1);
	const ringIdx = floor(__tsover_mul(rNorm, n));
	let coord = clamp(__tsover_div(__tsover_add(ringIdx, .5), n), 0, 1);
...
var componentDefinition = {
	...
	fragment: ({ uniforms, childNode, ctx }) => {
		if (!childNode) {
			console.error("You must pass a child component into the Ripple Wipe shader.");
			return ZERO;
		}
		return call(rippleWipeMask, "rippleWipeMask", [
			childNode,
			ctx.uv,
			ctx.aspect,
			uniforms.center,
...
//#endregion
//#region node_modules/shaders/dist/react/RippleWipe.js
var RippleWipe_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => RippleWipe_default
});
...
//#endregion
export { RippleWipe_exports as n, RippleWipe_default as t };
