...
import { C as ZERO, Cn as std_exports, P as call, Sn as src_default, Ut as resolveBoundingBox, Vn as dot, Yn as mix, _r as vec3f, cr as data_exports, f as transformColor, l as setColorSpaceMode, n as ShaderContext, qn as max, s as createGpuUniformsMap, vr as vec4f, yr as f32 } from "./Shader-BPX4e-H3.js";
import { t as require_jsx_runtime } from "./react_jsx-runtime.js";
import { t as require_react_dom } from "./react-dom.js";
//#region node_modules/shaders/dist/core/Tint-BTmcGjEl.js
var tintColorPlain = src_default.fn([
	vec4f,
	vec4f,
	f32
], vec4f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((color, tintColor, amount) => {
	"use gpu";
	const tinted = mix(vec3f(color.x, color.y, color.z), vec3f(tintColor.x, tintColor.y, tintColor.z), vec3f(amount));
	return vec4f(tinted.x, tinted.y, tinted.z, color.w);
}), {
	v: 1,
	name: void 0,
	ast: {
...
var tintColorPreserve = src_default.fn([
	vec4f,
	vec4f,
	f32
], vec4f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((color, tintColor, amount) => {
	"use gpu";
	const rgb = vec3f(color.x, color.y, color.z);
	const tinted = mix(rgb, vec3f(tintColor.x, tintColor.y, tintColor.z), vec3f(amount));
	const luminanceWeights = vec3f(.299, .587, .114);
	const originalLum = dot(rgb, luminanceWeights);
	const tintedLum = dot(tinted, luminanceWeights);
	const result = tinted.mul(__tsover_div(originalLum, max(tintedLum, 1e-4)));
	return vec4f(result.x, result.y, result.z, color.w);
}), {
	v: 1,
	name: void 0,
	ast: {
...
var componentDefinition = {
	...
	fragment: ({ uniforms, childNode, propValues }) => {
		...
		if (Number(propValues.preserveLuminosity) > 0) return call(tintColorPreserve, "tintColorPreserve", [
			childNode,
			uniforms.color,
			uniforms.amount
		]);
		return call(tintColorPlain, "tintColorPlain", [
			childNode,
			uniforms.color,
			uniforms.amount
		]);
...
var Tint_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => Tint_default
});
...
var Tint_default = ShaderComponent;
//#endregion
export { Tint_exports as n, Tint_default as t };
