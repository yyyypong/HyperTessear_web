...
import { Cn as std_exports, Gn as fract, P as call, Rn as clamp, Sn as src_default, Ut as resolveBoundingBox, Wn as floor, cr as data_exports, f as transformColor, hr as vec2f, ir as smoothstep, kn as fwidth, l as setColorSpaceMode, n as ShaderContext, qn as max, rr as sin, s as createGpuUniformsMap, vr as vec4f, yr as f32, zn as cos } from "./Shader-BPX4e-H3.js";
import { t as require_jsx_runtime } from "./react_jsx-runtime.js";
import { t as require_react_dom } from "./react-dom.js";
//#region node_modules/shaders/dist/core/Weave-D1D3kuUg.js
var DEG_TO_RAD = Math.PI / 180;
var weaveColor = src_default.fn([
	vec2f,
	vec2f,
	f32,
	f32,
	...
	vec4f,
	vec4f
], vec4f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((uv, viewport, cells, gap, rotation, colorA, colorB) => {
	"use gpu";
	...
	const hBand = __tsover_mul(smoothstep(gap, __tsover_add(gap, pixel), localUV.y), smoothstep(gap, __tsover_add(gap, pixel), __tsover_sub(1, localUV.y)));
	const vBand = __tsover_mul(smoothstep(gap, __tsover_add(gap, pixel), localUV.x), smoothstep(gap, __tsover_add(gap, pixel), __tsover_sub(1, localUV.x)));
	const s = __tsover_add(cell.x, cell.y);
	const isHTopFloat = __tsover_sub(s, __tsover_mul(2, floor(__tsover_div(s, 2))));
	const intersection = __tsover_mul(hBand, vBand);
	const hVis = clamp(__tsover_sub(hBand, __tsover_mul(intersection, isHTopFloat)), 0, 1);
	const vVis = clamp(__tsover_sub(vBand, __tsover_mul(intersection, __tsover_sub(1, isHTopFloat))), 0, 1);
	const weightA = __tsover_mul(colorA.w, hVis);
	const weightB = __tsover_mul(colorB.w, vVis);
	const totalAlpha = clamp(__tsover_add(weightA, weightB), 0, 1);
	const safeWeight = max(__tsover_add(weightA, weightB), .001);
	return vec4f(__tsover_div(__tsover_add(__tsover_mul(colorA.x, weightA), __tsover_mul(colorB.x, weightB)), safeWeight), __tsover_div(__tsover_add(__tsover_mul(colorA.y, weightA), __tsover_mul(colorB.y, weightB)), safeWeight), __tsover_div(__tsover_add(__tsover_mul(colorA.z, weightA), __tsover_mul(colorB.z, weightB)), safeWeight), totalAlpha);
}), {
	v: 1,
	name: void 0,
	ast: {
...
var componentDefinition = {
	...
	fragment: (params) => {
		...
		return call(weaveColor, "weaveColor", [
...
//#endregion
//#region node_modules/shaders/dist/react/Weave.js
var Weave_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => Weave_default
});
...
var Weave_default = ShaderComponent;
//#endregion
export { Weave_exports as n, Weave_default as t };
