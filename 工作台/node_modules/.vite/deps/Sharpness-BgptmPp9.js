...
import { C as ZERO, Cn as std_exports, P as call, Rn as clamp, Sn as src_default, Ut as resolveBoundingBox, _r as vec3f, cr as data_exports, hn as unpremultiplyAlpha, hr as vec2f, it as floatE, l as setColorSpaceMode, n as ShaderContext, s as createGpuUniformsMap, vr as vec4f, yr as f32 } from "./Shader-BPX4e-H3.js";
import { t as require_jsx_runtime } from "./react_jsx-runtime.js";
import { t as require_react_dom } from "./react-dom.js";
//#region node_modules/shaders/dist/core/Sharpness-IRuPd9nn.js
var sharpnessTapUV = src_default.fn([
	vec2f,
	vec2f,
	f32,
	f32
], vec2f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((uv, viewport, dirX, dirY) => {
	"use gpu";
	return vec2f(__tsover_add(uv.x, __tsover_div(dirX, viewport.x)), __tsover_add(uv.y, __tsover_div(dirY, viewport.y)));
}), {
	v: 1,
	name: void 0,
	ast: {
...
var sharpnessCompose = src_default.fn([
	vec4f,
	vec4f,
	vec4f,
	vec4f,
	vec4f,
	f32
], vec4f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((center, top, bottom, left, right, amount) => {
	"use gpu";
	const centerWeight = __tsover_add(1, __tsover_mul(amount, 4));
	const neighborWeight = __tsover_mul(amount, -1);
	const clamped = clamp(center.xyz.mul(centerWeight).add(top.xyz.mul(neighborWeight)).add(bottom.xyz.mul(neighborWeight)).add(left.xyz.mul(neighborWeight)).add(right.xyz.mul(neighborWeight)), vec3f(0, 0, 0), vec3f(1, 1, 1));
	return vec4f(clamped.x, clamped.y, clamped.z, center.w);
}), {
	v: 1,
	name: void 0,
	ast: {
...
var componentDefinition = {
	...
	fragment: ({ uniforms, childNode, ctx, propValues, convertToTexture }) => {
		...
		if ((propValues.sharpness ?? 0) === 0) return call(unpremultiplyAlpha, "unpremultiplyAlpha", [texture.sample(ctx.uv)]);
		const vp = ctx.viewportSize;
		return call(unpremultiplyAlpha, "unpremultiplyAlpha", [call(sharpnessCompose, "sharpnessCompose", [
			texture.sample(ctx.uv),
			texture.sample(call(sharpnessTapUV, "sharpnessTapUV", [
				ctx.uv,
				vp,
				floatE(0),
				floatE(1)
...
var Sharpness_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => Sharpness_default
});
...
var Sharpness_default = ShaderComponent;
//#endregion
export { Sharpness_exports as n, Sharpness_default as t };
