...
import { C as ZERO, Cn as std_exports, D as applyEdgeHandlingExpr, Fn as abs, Gn as fract, P as call, R as composeEdgeRemapExpr, Sn as src_default, Ut as resolveBoundingBox, Vn as dot, Wn as floor, cr as data_exports, hn as unpremultiplyAlpha, hr as vec2f, l as setColorSpaceMode, n as ShaderContext, qn as max, rr as sin, s as createGpuUniformsMap, yr as f32, zn as cos } from "./Shader-BPX4e-H3.js";
//#region node_modules/shaders/dist/core/SliceWipe-BOcCk5Se.js
var DEG_TO_RAD = Math.PI / 180;
var sliceWipeUV = src_default.fn([
	vec2f,
	f32,
	f32,
	f32,
	f32
], vec2f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((uv, aspect, angleDeg, sliceCount, progress) => {
	"use gpu";
	const a = __tsover_mul(angleDeg, DEG_TO_RAD);
	const dir = vec2f(cos(a), sin(a));
	const p = vec2f(__tsover_mul(__tsover_sub(uv.x, .5), aspect), __tsover_sub(uv.y, .5));
	const grad = dot(p, dir);
	const ext = __tsover_mul(.5, __tsover_add(__tsover_mul(aspect, abs(dir.x)), abs(dir.y)));
	const t = __tsover_add(__tsover_div(grad, __tsover_mul(2, ext)), .5);
	const idx = floor(__tsover_mul(t, max(sliceCount, 1)));
	const sign = __tsover_sub(__tsover_mul(fract(__tsover_mul(idx, .5)), 4), 1);
	const perp = vec2f(-dir.y, dir.x);
	const extPerp = __tsover_mul(.5, __tsover_add(__tsover_mul(aspect, abs(dir.y)), abs(dir.x)));
	const slide = __tsover_mul(__tsover_mul(progress, __tsover_mul(2.02, extPerp)), sign);
	const q = p.add(perp.mul(slide));
	return vec2f(__tsover_add(__tsover_div(q.x, aspect), .5), __tsover_add(q.y, .5));
}), {
	v: 1,
	name: void 0,
	ast: {
...
var componentDefinition = {
	...
	fragment: ({ uniforms, childNode, ctx, convertToTexture }) => {
		if (!childNode) {
			console.error("You must pass a child component into the Slice Wipe shader.");
			return ZERO;
		}
		const texture = convertToTexture(childNode);
		return call(unpremultiplyAlpha, "unpremultiplyAlpha", [applyEdgeHandlingExpr(call(sliceWipeUV, "sliceWipeUV", [
			ctx.uv,
			ctx.aspect,
			uniforms.angle,
			uniforms.sliceCount,
			uniforms.progress
		]), (uv) => texture.sample(uv), 1)]);
	},
	uvRemap: ({ uv, mask, uniforms, aspect }) => {
		return composeEdgeRemapExpr(call(sliceWipeUV, "sliceWipeUV", [
			uv,
			aspect,
			uniforms.angle,
			uniforms.sliceCount,
...
//#endregion
//#region node_modules/shaders/dist/react/SliceWipe.js
var SliceWipe_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => SliceWipe_default
});
...
//#endregion
export { SliceWipe_exports as n, SliceWipe_default as t };
