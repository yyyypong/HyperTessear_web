...
import { C as ZERO, Cn as std_exports, Kn as length, Ln as atan2, P as call, Pn as select, R as composeEdgeRemapExpr, Sn as src_default, Ut as resolveBoundingBox, Wn as floor, Yt as sampleRemappedExpr, cr as data_exports, h as transformPosition, hn as unpremultiplyAlpha, hr as vec2f, l as setColorSpaceMode, m as transformEdges, n as ShaderContext, rr as sin, s as createGpuUniformsMap, yr as f32, zn as cos } from "./Shader-BPX4e-H3.js";
//#region node_modules/shaders/dist/core/Kaleidoscope-C8_-tC6T.js
var DEG_TO_RAD = Math.PI / 180;
var TAU = Math.PI * 2;
var kaleidoscopeUV = src_default.fn([
	vec2f,
	f32,
	f32,
	vec2f,
	f32
], vec2f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((center, segments, angle, uv, aspect) => {
	"use gpu";
	...
	const rotatedAngle = __tsover_add(currentAngle, __tsover_mul(angle, DEG_TO_RAD));
	const segmentAngle = __tsover_div(TAU, segments);
	const segTwo = __tsover_mul(segmentAngle, 2);
	const wrappedAngle = __tsover_sub(rotatedAngle, __tsover_mul(segTwo, floor(__tsover_div(rotatedAngle, segTwo))));
	const finalAngle = select(wrappedAngle, __tsover_sub(segTwo, wrappedAngle), wrappedAngle > segmentAngle);
	const kaleidoscopeX = __tsover_mul(cos(finalAngle), radius);
	const kaleidoscopeY = __tsover_mul(sin(finalAngle), radius);
	const kaleidoscopePos = centerPos.add(vec2f(kaleidoscopeX, kaleidoscopeY));
	return vec2f(__tsover_div(kaleidoscopePos.x, aspect), kaleidoscopePos.y);
}), {
	v: 1,
	name: void 0,
	ast: {
...
var componentDefinition = {
	...
	fragment: ({ uniforms, childNode, ctx, propValues, convertToTexture }) => {
		if (!childNode) {
			console.error("You must pass a child component into the Kaleidoscope shader.");
			return ZERO;
		}
		const texture = convertToTexture(childNode);
		const edgeMode = propValues.edges ?? 0;
		return call(unpremultiplyAlpha, "unpremultiplyAlpha", [sampleRemappedExpr(texture, call(kaleidoscopeUV, "kaleidoscopeUV", [
			uniforms.center,
			uniforms.segments,
			uniforms.angle,
			ctx.uv,
	...
	uvRemap: ({ uv, mask, uniforms, aspect, propValues }) => {
		const edgeMode = propValues.edges ?? 0;
		return composeEdgeRemapExpr(call(kaleidoscopeUV, "kaleidoscopeUV", [
			uniforms.center,
			uniforms.segments,
			uniforms.angle,
			uv,
...
//#endregion
//#region node_modules/shaders/dist/react/Kaleidoscope.js
var Kaleidoscope_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => Kaleidoscope_default
});
...
//#endregion
export { Kaleidoscope_exports as n, Kaleidoscope_default as t };
