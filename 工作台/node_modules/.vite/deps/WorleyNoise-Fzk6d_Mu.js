...
import { Cn as std_exports, Ct as mixColorsLinear, Fn as abs, Gn as fract, Jn as min, P as call, Pn as select, Rn as clamp, Sn as src_default, Ut as resolveBoundingBox, Vn as dot, Wn as floor, Yn as mix, a as animatedTime, ar as sqrt, c as colorSpaceOptions, cr as data_exports, f as transformColor, hr as vec2f, it as floatE, l as setColorSpaceMode, n as ShaderContext, p as transformColorSpace, qn as max, rr as sin, s as createGpuUniformsMap, wt as mixColorsVariants, yr as f32, zn as cos } from "./Shader-BPX4e-H3.js";
//#region node_modules/shaders/dist/core/WorleyNoise-BAEo0duh.js
var TAU = Math.PI * 2;
var worleyHash = src_default.fn([vec2f], vec2f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((p) => {
	"use gpu";
	return fract(sin(vec2f(dot(p, vec2f(127.1, 311.7)), dot(p, vec2f(269.5, 183.3)))).mul(43758.5453));
}), {
	v: 1,
	name: void 0,
	ast: {
...
var worleyEvalOctave = src_default.fn([
	vec2f,
	f32,
	f32,
	f32,
	f32,
	f32
], f32)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((oUV, animT, seedOff, jitter, distNum, modeNum) => {
	"use gpu";
	...
	for (let ny = -1; ny <= 1; ny++) for (let nx = -1; nx <= 1; nx++) {
		const offset = vec2f(f32(nx), f32(ny));
		const h = worleyHash(vec2f(__tsover_add(__tsover_add(cell.x, offset.x), seedOff), __tsover_add(__tsover_add(cell.y, offset.y), seedOff)));
		const px = __tsover_add(h.x, __tsover_mul(sin(__tsover_add(animT, __tsover_mul(h.x, TAU))), .15));
		const py = __tsover_add(h.y, __tsover_mul(cos(__tsover_add(animTSlow, __tsover_mul(h.y, TAU))), .15));
		const jx = mix(.5, clamp(px, .05, .95), jitter);
		const jy = mix(.5, clamp(py, .05, .95), jitter);
		...
		const dManhattan = __tsover_add(abs(deltaX), abs(deltaY));
		const dChebyshev = max(abs(deltaX), abs(deltaY));
		let dd = dEuclid;
		dd = select(dd, dManhattan, distNum === 1);
		dd = select(dd, dChebyshev, distNum === 2);
		d2 = min(d2, max(dd, d1));
		d1 = min(d1, dd);
	}
	const isEuclid = distNum === 0;
	const sd1 = select(d1, sqrt(d1), isEuclid);
	const sd2 = select(d2, sqrt(d2), isEuclid);
	let raw = sd1;
	raw = select(raw, sd2, modeNum === 1);
	raw = select(raw, __tsover_sub(sd2, sd1), modeNum === 2);
	raw = select(raw, __tsover_add(sd1, sd2), modeNum === 3);
	raw = select(raw, __tsover_mul(sd1, sd2), modeNum === 4);
	return raw;
}), {
	v: 1,
	name: void 0,
...
var worleyNoiseField = src_default.fn([
	vec2f,
	vec2f,
	f32,
	f32,
	...
	f32,
	f32
], f32)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((uv, viewport, scale, seed, jitter, lacunarity, persistence, animTime, octaves, distNum, modeNum, modeScale) => {
	"use gpu";
	...
	for (let o = 0; o < 4; o++) {
		const fo = f32(o);
		const gate = select(f32(0), f32(1), fo < octaves);
		const raw = worleyEvalOctave(vec2f(__tsover_mul(baseUVx, freq), __tsover_mul(baseUVy, freq)), __tsover_add(animTime, __tsover_mul(fo, 17)), __tsover_add(seed, __tsover_mul(fo, 31)), jitter, distNum, modeNum);
	...
	return __tsover_mul(__tsover_div(acc, max(totalAmp, 1e-4)), modeScale);
}), {
	v: 1,
	name: void 0,
	ast: {
...
var worleyTone = src_default.fn([
	f32,
	f32,
	f32
], f32)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((normalized, contrast, balance) => {
	"use gpu";
	const signed = __tsover_sub(__tsover_mul(normalized, 2), 1);
	const adjusted = __tsover_add(__tsover_mul(signed, contrast), balance);
	return clamp(__tsover_add(__tsover_mul(adjusted, .5), .5), 0, 1);
}), {
	v: 1,
	name: void 0,
	ast: {
...
//#endregion
//#region node_modules/shaders/dist/react/WorleyNoise.js
var WorleyNoise_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => WorleyNoise_default
});
...
//#endregion
export { WorleyNoise_exports as n, WorleyNoise_default as t };
