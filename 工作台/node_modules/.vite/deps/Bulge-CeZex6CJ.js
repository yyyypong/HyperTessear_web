...
import { C as ZERO, Cn as std_exports, Kn as length, P as call, R as composeEdgeRemapExpr, Sn as src_default, Ut as resolveBoundingBox, Yt as sampleRemappedExpr, cr as data_exports, h as transformPosition, hn as unpremultiplyAlpha, hr as vec2f, ir as smoothstep, l as setColorSpaceMode, m as transformEdges, n as ShaderContext, qn as max, s as createGpuUniformsMap, yr as f32 } from "./Shader-BPX4e-H3.js";
import { t as require_jsx_runtime } from "./react_jsx-runtime.js";
import { t as require_react_dom } from "./react-dom.js";
//#region node_modules/shaders/dist/core/Bulge-C1Pq4DHk.js
var bulgeUV = src_default.fn([
	vec2f,
	f32,
	f32,
	f32,
	vec2f,
	f32
], vec2f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((center, strength, radius, falloff, uv, aspect) => {
	"use gpu";
	...
	const effectRadius = __tsover_mul(radius, .5);
	const innerRadius = __tsover_mul(effectRadius, max(__tsover_sub(__tsover_sub(1, falloff), .001), 0));
	const smoothFalloff = __tsover_sub(1, smoothstep(innerRadius, effectRadius, distance));
	const normalizedDist = __tsover_div(distance, effectRadius);
	const distSq = __tsover_mul(normalizedDist, normalizedDist);
	const quadraticFalloff = max(0, __tsover_sub(1, distSq));
	const falloffTotal = __tsover_mul(smoothFalloff, quadraticFalloff);
	const negStrength = __tsover_mul(strength, -1);
	const displacementAmount = __tsover_mul(negStrength, falloffTotal);
	const scaleFactor = __tsover_add(1, displacementAmount);
	const bulgedDelta = delta.mul(scaleFactor);
	const bulgedUV = centerPos.add(bulgedDelta);
	return vec2f(__tsover_div(bulgedUV.x, aspect), bulgedUV.y);
}), {
	v: 1,
	name: void 0,
	ast: {
...
var componentDefinition = {
	...
	fragment: ({ uniforms, childNode, ctx, propValues, convertToTexture }) => {
		if (!childNode) {
			console.error("You must pass a child component into the Bulge shader.");
			return ZERO;
		}
		const texture = convertToTexture(childNode);
		const edgeMode = propValues.edges ?? 0;
		return call(unpremultiplyAlpha, "unpremultiplyAlpha", [sampleRemappedExpr(texture, call(bulgeUV, "bulgeUV", [
			uniforms.center,
			uniforms.strength,
			uniforms.radius,
			uniforms.falloff,
	...
	uvRemap: ({ uv, mask, uniforms, aspect, propValues }) => {
		const edgeMode = propValues.edges ?? 0;
		return composeEdgeRemapExpr(call(bulgeUV, "bulgeUV", [
			uniforms.center,
			uniforms.strength,
			uniforms.radius,
			uniforms.falloff,
...
var Bulge_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => Bulge_default
});
...
//#endregion
export { Bulge_exports as n, Bulge_default as t };
