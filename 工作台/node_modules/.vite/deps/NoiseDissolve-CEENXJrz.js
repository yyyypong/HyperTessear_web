...
import { C as ZERO, Cn as std_exports, Mt as noise_exports, P as call, Pn as select, Rn as clamp, Sn as src_default, Ut as resolveBoundingBox, cr as data_exports, d as transformBoolean, hr as vec2f, ir as smoothstep, kt as mxNoiseFloat2, l as setColorSpaceMode, n as ShaderContext, qn as max, s as createGpuUniformsMap, vr as vec4f, yr as f32 } from "./Shader-BPX4e-H3.js";
import { t as require_jsx_runtime } from "./react_jsx-runtime.js";
import { t as require_react_dom } from "./react-dom.js";
//#region node_modules/shaders/dist/core/NoiseDissolve-CnELvUG_.js
var noiseDissolveMask = src_default.fn([
	vec4f,
	vec2f,
	f32,
	f32,
	...
	f32,
	f32
], vec4f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((color, uv, aspect, scale, softness, seed, progress, invert) => {
	"use gpu";
	const p = vec2f(__tsover_add(__tsover_mul(__tsover_mul(uv.x, aspect), scale), __tsover_mul(seed, 13.7)), __tsover_add(__tsover_mul(uv.y, scale), __tsover_mul(seed, 7.9)));
	const n1 = mxNoiseFloat2(p);
	const n2 = __tsover_mul(mxNoiseFloat2(vec2f(__tsover_add(__tsover_add(__tsover_mul(p.x, 2), 17.3), __tsover_mul(seed, 3.1)), __tsover_add(__tsover_mul(p.y, 2), 9.1))), .5);
	const n3 = __tsover_mul(mxNoiseFloat2(vec2f(__tsover_add(__tsover_mul(p.x, 4), 41.7), __tsover_add(__tsover_add(__tsover_mul(p.y, 4), 27.9), __tsover_mul(seed, 5.3)))), .25);
	const n = __tsover_div(__tsover_add(__tsover_add(n1, n2), n3), 1.75);
	let r = clamp(__tsover_add(.5, __tsover_mul(n, .9)), 0, 1);
	r = select(r, __tsover_sub(1, r), invert > 0);
	const f = max(softness, 1e-4);
	const front = __tsover_sub(__tsover_mul(progress, __tsover_add(1, __tsover_mul(2, f))), f);
	const reveal = smoothstep(__tsover_sub(front, f), __tsover_add(front, f), r);
	return vec4f(color.x, color.y, color.z, __tsover_mul(color.w, reveal));
}), {
	v: 1,
	name: void 0,
	ast: {
...
var componentDefinition = {
	...
	fragment: ({ uniforms, childNode, ctx }) => {
		if (!childNode) {
			console.error("You must pass a child component into the Noise Dissolve shader.");
			return ZERO;
		}
		return call(noiseDissolveMask, "noiseDissolveMask", [
			childNode,
			ctx.uv,
			ctx.aspect,
			uniforms.scale,
...
//#endregion
//#region node_modules/shaders/dist/react/NoiseDissolve.js
var NoiseDissolve_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => NoiseDissolve_default
});
...
//#endregion
export { NoiseDissolve_exports as n, NoiseDissolve_default as t };
