...
//#region node_modules/shaders/dist/core/Twirl-Cw4jm59Q.js
var twirlUV = src_default.fn([
	vec2f,
	f32,
	vec2f,
	f32
], vec2f)(/* @__PURE__ */ (($) => (globalThis.__TYPEGPU_META__ ??= /* @__PURE__ */ new WeakMap()).set($.f = ((center, intensity, uv, aspect) => {
	"use gpu";
	const centerPos = vec2f(center.x, __tsover_sub(1, center.y));
	const delta = uv.sub(centerPos);
	const acd = vec2f(__tsover_mul(delta.x, aspect), delta.y);
	const angle = __tsover_mul(intensity, length(acd));
	const cosA = cos(angle);
	const sinA = sin(angle);
	const rotatedX = __tsover_sub(__tsover_mul(cosA, acd.x), __tsover_mul(sinA, acd.y));
	const rotatedY = __tsover_add(__tsover_mul(sinA, acd.x), __tsover_mul(cosA, acd.y));
	return vec2f(__tsover_add(__tsover_div(rotatedX, aspect), centerPos.x), __tsover_add(rotatedY, centerPos.y));
}), {
	v: 1,
	name: void 0,
	ast: {
...
var componentDefinition = {
	...
	fragment: ({ uniforms, childNode, ctx, propValues, convertToTexture }) => {
		...
		return call(unpremultiplyAlpha, "unpremultiplyAlpha", [sampleRemappedExpr(texture, call(twirlUV, "twirlUV", [
			uniforms.center,
			uniforms.intensity,
			ctx.uv,
			ctx.aspect
	...
	uvRemap: ({ uv, mask, uniforms, aspect, propValues }) => {
		const edgeMode = propValues.edges ?? 0;
		return composeEdgeRemapExpr(call(twirlUV, "twirlUV", [
			uniforms.center,
			uniforms.intensity,
			uv,
			aspect
...
var Twirl_exports = /* @__PURE__ */ __exportAll({
	ShaderComponent: () => ShaderComponent,
	default: () => Twirl_default
});
...
var Twirl_default = ShaderComponent;
//#endregion
export { Twirl_exports as n, Twirl_default as t };
