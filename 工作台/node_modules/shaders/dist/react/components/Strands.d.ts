import { default as React } from 'react';
import { BlendMode, PropDriver, TransformConfig, BoundingBoxConfig, LayoutConfig } from '../../core';
import { ComponentProps } from '../../core/shaders/Strands';

export type { ComponentProps };
/**
 * Base props that all shader components have
 */
interface BaseShaderProps {
    children?: React.ReactNode;
    blendMode?: BlendMode;
    opacity?: number;
    visible?: boolean;
    id?: string;
    maskSource?: string;
    maskType?: string;
    renderOrder?: number;
    transform?: Partial<TransformConfig>;
    boundingBox?: Partial<BoundingBoxConfig>;
    flow?: LayoutConfig;
    absolute?: boolean;
    ref?: React.Ref<any>;
}
/**
 * Component-specific props that merge base props with shader-specific props
 * Note: ComponentProps are made optional since they have defaults from the shader definition
 */
type ShaderComponentProps = BaseShaderProps & Partial<Omit<ComponentProps, 'amplitude' | 'frequency' | 'lineWidth' | 'softness' | 'spread' | 'colorScale' | 'colorVariance' | 'start' | 'end'>> & {
    amplitude?: ComponentProps['amplitude'] | PropDriver;
    frequency?: ComponentProps['frequency'] | PropDriver;
    lineWidth?: ComponentProps['lineWidth'] | PropDriver;
    softness?: ComponentProps['softness'] | PropDriver;
    spread?: ComponentProps['spread'] | PropDriver;
    colorScale?: ComponentProps['colorScale'] | PropDriver;
    colorVariance?: ComponentProps['colorVariance'] | PropDriver;
    start?: ComponentProps['start'] | PropDriver;
    end?: ComponentProps['end'] | PropDriver;
};
/**
 * The main React wrapper component for Shader shader nodes
 */
export declare const ShaderComponent: React.FC<ShaderComponentProps>;
export default ShaderComponent;
//# sourceMappingURL=Strands.d.ts.map