import { JSX } from 'solid-js';
import { BlendMode, PropDriver, TransformConfig, BoundingBoxConfig, LayoutConfig } from '../../core';
import { ComponentProps } from '../../core/shaders/MultiPointGradient';
export type { ComponentProps };
/**
 * Base props that all shader components have
 */
interface BaseShaderProps {
    children?: JSX.Element;
    blendMode?: BlendMode;
    opacity?: number;
    visible?: boolean;
    id?: string;
    maskSource?: string;
    maskType?: string;
    renderOrder?: number;
    transform?: Partial<TransformConfig>;
    boundingBox?: Partial<BoundingBoxConfig>;
    flow?: LayoutConfig;
    absolute?: boolean;
}
/**
 * Component-specific props that merge base props with shader-specific props
 * Note: ComponentProps are made optional since they have defaults from the shader definition
 */
type ShaderComponentProps = BaseShaderProps & Partial<Omit<ComponentProps, 'positionA' | 'positionB' | 'positionC' | 'positionD' | 'positionE' | 'smoothness'>> & {
    positionA?: ComponentProps['positionA'] | PropDriver;
    positionB?: ComponentProps['positionB'] | PropDriver;
    positionC?: ComponentProps['positionC'] | PropDriver;
    positionD?: ComponentProps['positionD'] | PropDriver;
    positionE?: ComponentProps['positionE'] | PropDriver;
    smoothness?: ComponentProps['smoothness'] | PropDriver;
};
/**
 * The main Solid wrapper component for Shader shader nodes
 */
export default function ShaderComponent(props: ShaderComponentProps): JSX.Element;
//# sourceMappingURL=MultiPointGradient.d.ts.map