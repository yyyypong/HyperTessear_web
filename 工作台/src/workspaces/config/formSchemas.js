...
export const FORM_SCHEMAS = Object.freeze({
  ...
  'vault.roles.set': { id: 'vault.roles.set', fields: [vault(), field('role', 'select', { options: ['curator', 'guardian', 'allocator', 'keeper'] }), account()] },
  'vault.settlement.configure': { id: 'vault.settlement.configure', fields: [vault(), field('operators', 'bytes-array'), field('threshold', 'integer', { validation: 'positive', validationMeta: { min: 1 } })] },
  'vault.modules.bind': { id: 'vault.modules.bind', fields: [vault(), field('settlement', 'address'), field('unifiedPool', 'address'), field('gate', 'address'), field('navSigner', 'address')] },
