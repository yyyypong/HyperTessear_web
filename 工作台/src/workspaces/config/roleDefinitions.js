...
export const ROLE_DEFINITIONS = Object.freeze({
  governor: role('governor', '/workspaces/governor', 'protocol', ['governor.members.manage', 'protocol.modules.pause', 'psm.protocol.pause', 'revenue.treasury.set', 'nav.signer.manage', 'protocol.fee.config', 'revenue.withdraw', 'revenue.withdraw.token', 'revenue.sources.manage', 'nav.deviation.set']),
  ...
  curator: role('curator', '/workspaces/curator/:vault', 'vault', ['vault.fees.set', 'vault.adapters.manage', 'vault.orders.manage', 'vault.data-policy.set', 'request.mark-refundable', 'claim.record', 'vault.evict-deposit']),
  ...
  'settlement-operator': role('settlement-operator', '/workspaces/settlement-operator/:vault', 'vault', ['settlement.instruction.sign', 'settlement.return-principal']),
  keeper: role('keeper', '/workspaces/keeper/:vault', 'vault', ['lifecycle.open-subscription', 'lifecycle.finalize-subscription', 'lifecycle.start-calculation', 'lifecycle.enter-final-settlement', 'lifecycle.enter-maturing', 'lifecycle.enter-claiming', 'lifecycle.close-product']),
...
export const ACTION_DEFINITIONS = Object.freeze({
  ...
  'revenue.treasury.set': action('revenue.treasury.set', 'protocol', 'RevenuePool.setYieldStrategy', ['RevenuePool'], 'setRevenueTreasury', ['RevenuePool']),
  'nav.signer.manage': action('nav.signer.manage', 'protocol', 'NAVOracle.addAuthorizedSigner/removeAuthorizedSigner', ['NAVOracle'], 'manageNavSigner', ['TargetNAVOracle']),
  'protocol.fee.config': action('protocol.fee.config', 'protocol', null, ['BaseVault'], 'setProtocolFeeConfig', ['VaultFeeController']),
  'revenue.withdraw': action('revenue.withdraw', 'protocol', 'RevenuePool.withdraw', ['RevenuePool'], 'withdrawRevenue', ['RevenuePool']),
  'revenue.withdraw.token': action('revenue.withdraw.token', 'protocol', null, ['RevenuePool'], 'withdrawRevenueToken', ['RevenuePool']),
  'revenue.sources.manage': action('revenue.sources.manage', 'protocol', 'RevenuePool.addAuthorizedSource/removeAuthorizedSource', ['RevenuePool'], 'manageRevenueSource', ['RevenuePool']),
  'nav.deviation.set': action('nav.deviation.set', 'protocol', null, ['NAVOracle'], 'setNavDeviationMaxBps', ['TargetNAVOracle']),
  'vault.roles.set': action('vault.roles.set', 'vault', 'BaseVault.setOperator', ['BaseVault'], 'setVaultRole', ['VaultRoles']),
  'vault.settlement.configure': action('vault.settlement.configure', 'vault', 'Settlement.addOperator/removeOperator/setThreshold', ['Settlement'], 'configureVaultSettlement', ['VaultSettlementConfig']),
  'vault.modules.bind': action('vault.modules.bind', 'vault', 'BaseVault.setSettlement/setUnifiedPool/setGate; NAVOracle.addAuthorizedSigner', ['BaseVault', 'NAVOracle'], 'bindVaultModules', ['VaultModuleRegistry']),
  'vault.adapters.configure': action('vault.adapters.configure', 'vault', 'BaseVault.addAdapter/removeAdapter', ['BaseVault'], 'configureVaultAdapter', ['AdapterRegistry']),
  ...
  'vault.adapters.manage': action('vault.adapters.manage', 'vault', 'BaseVault.addAdapter/removeAdapter', ['BaseVault'], 'manageVaultAdapter', ['AdapterRegistry']),
  ...
  'settlement.return-principal': action('settlement.return-principal', 'vault', null, ['BaseVault'], 'returnPrincipalToPool', ['VaultSettlementConfig']),
  'vault.evict-deposit': action('vault.evict-deposit', 'vault', null, ['LiquidityEarnVault'], 'evictDepositRequest', ['VaultRequestManager']),
  ...
  'psm.authorization.sign': action('psm.authorization.sign', 'asset', 'signLegacyPsmDigest', ['ReservePSM'], 'signPsmAuthorization', ['ReservePSM']),
  ...
  'psm.authorization.submit': action('psm.authorization.submit', 'permissionless', 'ReservePSM.mintWithAuthorization', ['ReservePSM'], 'submitPsmAuthorization', ['ReservePSM']),
